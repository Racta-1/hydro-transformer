...
class Informer(BaseModel):
    """Informer model class with fixes for river routing.

    Key improvements:
    1. Optional input scaling (can be disabled)
    2. Cached causal mask (not recreated every forward)
    3. Mask actually passed to encoder
    4. Correct config parameters used
    5. Better logging
    ...
    def __init__(self, cfg: Config):
        ...
        dropout = getattr(cfg, 'informer_dropout', 0.1)
        ...
        LOGGER.info(f'Encoder layers: {e_layers}')
        LOGGER.info(f'Attention heads: {n_heads}')
        LOGGER.info(f'ProbSparse factor: {factor}')
        LOGGER.info(f'Activation: {activation}')
        LOGGER.info(f'Distillation: {distil}')
        LOGGER.info(f'Dropout: {dropout}')
        LOGGER.info('=' * 50)

        self.embedding_net = InputLayer(cfg)

        if self.embedding_net.output_size % n_heads != 0:
            raise ValueError(
                f"Embedding dimension ({self.embedding_net.output_size}) must be divisible by "
                f"number of Informer heads ({n_heads}). "
                "Use statics_embedding/dynamics_embedding to adjust embedding size."
            )

        self._scale_inputs = getattr(cfg, 'informer_scale_inputs', False)
        if self._scale_inputs:
            self._sqrt_embedding_dim = math.sqrt(self.embedding_net.output_size)
            LOGGER.info(f"Input scaling enabled: multiplying by {self._sqrt_embedding_dim:.2f}")
        else:
            self._sqrt_embedding_dim = 1.0
            LOGGER.info("Input scaling disabled")
        ...
        LOGGER.info(f"Causal mask cached: shape {mask.shape}")
        ...
        self.encoder = Encoder(
            ...
            [
                ConvLayer(encoder_dim) for l in range(e_layers - 1)
            ] if distil else None,
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """Perform a forward pass on Informer model.
        ...
        if self._scale_inputs:
            x_scaled = x_d * self._sqrt_embedding_dim
        else:
            x_scaled = x_d
        
        positional_encoding = self.positional_encoder(x_scaled)
        
        positional_encoding = positional_encoding.transpose(0, 1)
        
        seq_len = positional_encoding.size(1)
        mask = self.causal_mask[:seq_len, :seq_len] if hasattr(self, 'causal_mask') else None

        output, attns = self.encoder(positional_encoding, attn_mask=mask)
...
class _PositionalEncoding(nn.Module):
    """Positional encoding for Informer.
    ...
    dropout : float
        Dropout rate applied to the embedding vector.
    position_type : str
        'sum' or 'concatenate'
    max_len : int
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        -------
        torch.Tensor
            Shape: [sequence length, batch size, encoder dimension]
        """
